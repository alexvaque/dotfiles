...
server = 'X.X.X.X'
credentials = pika.PlainCredentials('sensu', 'pass')
...
def send_mail_error(error_message):
    ...
    ret = conn.send_email('rundeck@mail.com',
                    'Sensu is not working properly with RABBITMQ',
                    'Sensu does not runs properly with RABBITMQ  , ALERT produced doing ...  ' + str(error_message) + " ",
                    ['alerts@mail.com'])
    return ret
